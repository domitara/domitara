...
import {
  Paper, Title, Text, Button, Group, Checkbox, Badge,
  ActionIcon, Loader, Center, Select, Tooltip,
} from '@mantine/core';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <Group justify="space-between">
        <div>
          <Title order={1} style={{ fontSize: '1.75rem' }}>Asset IDs</Title>
          {withoutId > 0 && (
            <Text size="xs" c="dimmed" mt={2}>{withoutId} item{withoutId !== 1 ? 's' : ''} without an asset ID</Text>
          )}
        </div>
        <Group gap={8}>
          {withoutId > 0 && (
      ...
      <Paper withBorder radius="md" style={{ overflow: 'hidden' }}>
        <div style={{
          ...
          color: 'var(--dt-fg-3)', letterSpacing: '.04em',
          alignItems: 'center',
        }}>
          <Checkbox
            size="xs"
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center' }}>
            ...
            <Text size="sm" c="dimmed">No items match this filter.</Text>
          </div>
        ) : filtered.map(item => (
          <AssetRow
            key={item.id}
...
function AssetRow({ item, checked, onToggle, onNavigate }: {
  item: Item;
  checked: boolean;
  onToggle: () => void;
  onNavigate: () => void;
  ...
  return (
    <div
      ...
      onMouseEnter={e => { if (!checked) e.currentTarget.style.background = 'var(--dt-gray-0)'; }}
      onMouseLeave={e => { if (!checked) e.currentTarget.style.background = ''; }}
    >
      <Checkbox size="xs" checked={checked} onChange={onToggle} onClick={e => e.stopPropagation()}/>
      ...
      <Text size="xs" c="dimmed" truncate>—</Text>
